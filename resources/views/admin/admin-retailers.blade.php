...
@section('adminDashboardContent')
    ...
    <div class="p-6">
        <h1 class="text-3xl font-bold mb-6 text-teal-600 dark:text-teal-400">Retailers Management</h1>
                ...
                <tbody>
                    @forelse ($retailers as $retailer)
                        <tr class="border-b bg-white dark:bg-gray-900 hover:bg-teal-50 dark:hover:bg-gray-700 transition">
                            <td class="p-3 text-gray-800 dark:text-gray-100">{{ $retailer->firstName }} {{ $retailer->lastName }}</td>
                            <td class="p-3 text-gray-800 dark:text-gray-100">{{ $retailer->email }}</td>
                            <td class="p-3 text-gray-800 dark:text-gray-100">{{ $retailer->created_at->format('Y-m-d') }}</td>

                            <td class="p-3 flex gap-2">
                                <form action="{{ route('admin.retailers.revoke', $retailer->id) }}" method="POST">
                                    @csrf
                                    <button onclick="return confirm('Demote to user?')"
                                        class="bg-red-700 text-white px-3 py-1 rounded hover:bg-red-600 transition">
                                        Demote
                                    </button>
                                </form>
